...
# How it works
Atlas is a bot that works through multiple subsystems that all work together to detect, identify and locate objects. The first subsystem is the Raspberry pi. the Pi is connected to a camera located at the base of the robot arm, and when the camera sees an object, it evaluates it using OpenCV to see it the size is small enough to be picked up by the claw. Next it located the object using pixels on the cameras frames, trained to accuracy using a data table of pixel rages to distances in mm. After it identifies the location of the object, it translates it into x and y coordinates, which it then sends to the Arduino in the form of "MOVE X Y Z" (z is always 0.00mm because it has to reach objects on the ground). After this, the Arduino calculates where the arm should move using inverse kinematics and equations θbase = atan2(X, Y), the distance from the object using the pythagorean theorem, because the Arduino treats the arm segments as x and y components of a right triangle where the line from the base to the object is the hypotenuse. It then calculates the distance, and calculates the elbow angle (using cos_theta2 = (R^2 - L1^2 - L2^2) / (2 * L1 * L2) and theta_2 = acos(max(-1.0, min(1.0, cos_theta2)))). It also calculates the distance from the shoulder to the object using the pythagorean theorem, and finds the angle of the shoulder using the equations cos_beta = (R^2 + L1^2 - L2^2) / (2 * R * L1), beta = acos(max(-1.0, min(1.0, cos_beta))), and theta_1 = alpha + beta where theta1 is the intermediate angle of the shoulder joint. This is how the bot effectively tracks objects and converts the location of said object into physical movement of the robotic arm using physics, inverse kinematics, and a lot of trigonometry.
